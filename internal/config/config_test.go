	...
	tests := []struct {
		...
		},
		{
			...
			cfg: &Config{
				...
				GrafanaSaToken: "testtoken",
				RunMode:        OneTime,
				BranchStrategy: BranchStrategy("invalid"),
			},
			wantErr: true,
		},
		{
			name: "Invalid sync interval in periodic mode",
			cfg: &Config{
				SSHURL:         "git@github.com:test/repo.git",
				SSHKey:         sshKeyPath,
				SSHUser:        "testuser",
		...
		{
			name: "Invalid branch TTL with reuse strategy",
			cfg: &Config{
				SSHURL:         "git@github.com:test/repo.git",
				SSHKey:         sshKeyPath,
				SSHUser:        "testuser",
